...
public class NoteJsonDeserializer extends AsyncOperation<String, ArrayList<Note>> {
    ...
    protected ArrayList<Note> doInBackground(String s) {
        try {
            JSONArray array = new JSONObject(s).getJSONArray(IConstant.NOTES);
            ArrayList<Note> result = new ArrayList<>();
            for (int i = 0; i < array.length(); i++) {
                JSONObject currentNote = array.getJSONObject(i);
                result.add(new Note(currentNote.getString(IConstant.TITLE), currentNote.getString(IConstant.MESSAGE), Note.Category.valueOf(currentNote.getString("category")), currentNote.getLong("id"), currentNote.getLong("date")));
            }
            return result;
